/**
 * Momentum Continuation Integration — tests (Sprint 11B.3F).
 */
...
function bullCandles(): MomentumContinuationCandle[] {
  return [
...
function bullDetection(
  overrides: Partial<MomentumContinuationDetection> = {}
  ...
  return {
    ...
    confidence: 85,
    reasons: ["Momentum Continuation BUY detected."],
    warnings: [],
    ...overrides,
  };
}
...
function makeInput(
  candles: MomentumContinuationCandle[] = bullCandles()
...
describe("Momentum Continuation Integration", () => {
  beforeEach(() => {
    resetStrategyEngine();
    resetMomentumContinuationMetrics();
    ensureMomentumContinuationRegistered();
  ...
  it("registers with StrategyRegistry", () => {
    const status = getMomentumContinuationIntegrationStatus();
    expect(status.registered).toBe(true);
    expect(status.strategyId).toBe(MOMENTUM_CONTINUATION_STRATEGY_ID);
    expect(getStrategyRegistry().has(MOMENTUM_CONTINUATION_STRATEGY_ID)).toBe(
      true
    );
  });

  it("creates instance via StrategyFactory", () => {
    expect(getStrategyFactory().has(MOMENTUM_CONTINUATION_STRATEGY_ID)).toBe(
      true
    );
    const instance = getStrategyFactory().create(
      MOMENTUM_CONTINUATION_STRATEGY_ID
    );
    expect(instance?.id).toBe(MOMENTUM_CONTINUATION_STRATEGY_ID);
  });

  it("executes through StrategyEngine", () => {
    const result = executeMomentumContinuationThroughEngine(makeContext(), {
      skipEligibilityCheck: true,
    });
    expect(result.signal.strategyId).toBe(MOMENTUM_CONTINUATION_STRATEGY_ID);
  });
  ...
  it("integrates with TradingPipeline adapter", () => {
    ...
    const result = executeMomentumContinuationWithPipeline(
      pipeline,
      makeInput(),
      { skipEligibilityCheck: true }
    );
  ...
  it("collects Metrics", () => {
    ...
    const snap = getMomentumContinuationMetrics().getSnapshot();
  ...
  it("scores Signal Grade", () => {
    const grade = calculateMomentumContinuationSignalGrade({
      conviction: 88,
      qualityScore: 82,
      riskReward: 2.5,
      marketStrength: 70,
  ...
  it("builds Explainability and Summary", () => {
    ...
    const score = buildMomentumContinuationInstitutionalScore({
      detection,
      setup,
      marketContext: makeMarketContext(),
      mcInput: makeInput(),
    });
    const explain = buildMomentumContinuationExplainability({
      detection,
      setup,
      marketContext: makeMarketContext(),
      mcInput: makeInput(),
    ...
    const summary = buildMomentumContinuationSummary({
      detection,
      setup,
  ...
  it("registerMomentumContinuationStrategy is idempotent-safe via ensure", () => {
    const registry = getStrategyRegistry();
    expect(ensureMomentumContinuationRegistered()).toBe(true);
    expect(ensureMomentumContinuationRegistered()).toBe(true);
    expect(registry.has(MOMENTUM_CONTINUATION_STRATEGY_ID)).toBe(true);
    const dup = registerMomentumContinuationStrategy(registry);
