/**
 * Relative Strength Intraday Risk utilities — Sprint 11B.3G.
 */
...
import {
  ...
  type RelativeStrengthIntradayStopMethod,
  type RelativeStrengthIntradayTradeConfig,
} from "./RelativeStrengthIntradayTradeTypes";

export interface RelativeStrengthIntradayStopCandidate {
  method: Exclude<RelativeStrengthIntradayStopMethod, "hybrid">;
  stopLoss: number;
  risk: number;
}
...
export function resolveStopLoss(input: {
  ...
  atr: number | null;
  candles: readonly RelativeStrengthIntradayCandle[];
  method?: RelativeStrengthIntradayStopMethod;
  config?: RelativeStrengthIntradayTradeConfig;
}): {
  stopLoss: number | null;
  method: RelativeStrengthIntradayStopMethod;
  candidates: RelativeStrengthIntradayStopCandidate[];
  warnings: string[];
} {
  const config = input.config ?? DEFAULT_RELATIVE_STRENGTH_INTRADAY_TRADE_CONFIG;
  const method = input.method ?? config.stopMethod;
  const warnings: string[] = [];
  const candidates: RelativeStrengthIntradayStopCandidate[] = [];

  const pushIfValid = (
    stopMethod: Exclude<RelativeStrengthIntradayStopMethod, "hybrid">,
    stop: number | null,
    softWarn: string
  ) => {
    if (
