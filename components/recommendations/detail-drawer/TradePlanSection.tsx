  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
        <MetricChip label="Recommended Entry" value={entryLabel} emphasize="entry" />
        <MetricChip
          label="Stop Loss"
        ...
        <MetricChip
          label="Risk / Reward"
          value={view.riskReward != null ? `${view.riskReward.toFixed(2)}x` : "—"}
        />
      ...
      <div className="mt-2 rounded-lg border border-sky-500/20 bg-sky-500/8 px-2.5 py-2">
        ...
        <p className="mt-0.5 text-[11px] text-text-secondary">{view.expiryLabel}</p>
      </div>
      {!view.available ? (
        <p className="mt-2 text-[11px] text-text-muted">
          Trade levels unavailable for this entry point — open from a full
          recommendation row for complete plan data.
        </p>
      ) : null}
    </SectionShell>
  );
