/**
 * Relative Strength Leadership Validator — Sprint 11B.3O.
 */
...
export class RelativeStrengthLeadershipValidator {
  ...
  validate(
    ...
    if (!context) {
      return {
        valid: false,
        errors: ["Relative Strength Leadership detection context is missing."],
        warnings: [],
      };
    }
    ...
    if (!data) {
      return {
        valid: false,
        errors: ["Missing Relative Strength Leadership market data payload."],
        warnings: [],
      };
    }
    ...
    if (probeTime && !isValidMarketHours(probeTime, this.config)) {
      warnings.push(
        "Market Hours soft-check failed for Relative Strength Leadership."
      );
    }
    ...
    if (
      ...
      errors.push(
        `Compatible Regime failed — ${context.regime.regime} blocked for RS Leadership.`
      );
    }

    const vol = context.marketContext?.volatility?.score;
    if (Number.isFinite(vol) && vol! > this.config.maxVolatilityScore) {
      errors.push("Volatility too high for Relative Strength Leadership.");
    }
...
export function createRelativeStrengthLeadershipValidator(
  config?: Partial<RelativeStrengthLeadershipConfig>
): RelativeStrengthLeadershipValidator {
  return new RelativeStrengthLeadershipValidator(
    config ?? DEFAULT_RELATIVE_STRENGTH_LEADERSHIP_CONFIG
  );
}
