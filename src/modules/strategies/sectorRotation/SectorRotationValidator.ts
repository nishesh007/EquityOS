/**
 * Sector Rotation Validator — Sprint 11B.3J.
 */

import { isStrategyEligible } from "../StrategyUtils";
import {
  DEFAULT_SECTOR_ROTATION_CONFIG,
  SECTOR_ROTATION_STRATEGY_ID,
  type SectorRotationConfig,
} from "./SectorRotationConstants";
import type {
  SectorRotationDetectionContext,
  SectorRotationValidationResult,
} from "./SectorRotationTypes";
import {
  isValidMarketHours,
  resolveSectorRotationConfig,
} from "./SectorRotationUtils";

export class SectorRotationValidator {
  private readonly config: SectorRotationConfig;

  constructor(config?: Partial<SectorRotationConfig>) {
    this.config = resolveSectorRotationConfig(config);
  }

  validate(
    context: SectorRotationDetectionContext | null | undefined
  ): SectorRotationValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];

    if (!context) {
      return {
        valid: false,
        errors: ["Sector Rotation detection context is missing."],
        warnings: [],
      };
    }

    const data = context.input?.sectorRotation;
    if (!data) {
      return {
        valid: false,
        errors: ["Missing Sector Rotation market data payload."],
        warnings: [],
      };
    }

    const candles = data.candles5m ?? [];
    if (candles.length < this.config.minimumSessionCandles) {
      errors.push("Enough Candles missing — insufficient 5-minute OHLC sample.");
    }

    if (data.ema20 === null || !Number.isFinite(data.ema20)) {
      errors.push("EMA20 missing.");
    }
    if (data.ema50 === null || !Number.isFinite(data.ema50)) {
      errors.push("EMA50 missing.");
    }
    if (!Number.isFinite(data.vwap) || data.vwap <= 0) {
      errors.push("Valid VWAP missing.");
    }

    if (!data.sectorName || data.sectorName.trim().length === 0) {
      warnings.push("Sector name missing — will use empty label.");
    }

    if (!context.marketContext) {
      errors.push("Valid Context missing — market context absent.");
    }
    if (!context.regime?.regime) {
      errors.push("Compatible Regime missing — market regime absent.");
    }
    if (!context.confidence || !Number.isFinite(context.confidence.score)) {
      errors.push("Valid Context incomplete — regime confidence missing.");
    }

    const eligible = isStrategyEligible(
      SECTOR_ROTATION_STRATEGY_ID,
      context.eligibleStrategies ?? []
    );
    if (!eligible) {
      errors.push("Eligible Strategy gate failed for Sector Rotation.");
    }

    const probeTime =
      context.timestamp ?? candles[candles.length - 1]?.timestamp ?? null;
    if (probeTime && !isValidMarketHours(probeTime, this.config)) {
      errors.push("Market Hours invalid for Sector Rotation detection.");
    }

    if (
      context.regime?.regime &&
      this.config.blockedRegimes.includes(context.regime.regime)
    ) {
      errors.push(
        `Compatible Regime failed — ${context.regime.regime} blocked for sector rotation.`
      );
    }

    const vol = context.marketContext?.volatility?.score;
    if (Number.isFinite(vol) && vol! > this.config.maxVolatilityScore) {
      errors.push("Volatility too high for sector rotation.");
    }

    if (
      data.relativeVolume !== null &&
      Number.isFinite(data.relativeVolume) &&
      data.relativeVolume < this.config.minRelativeVolume
    ) {
      warnings.push("Low relative volume.");
    }

    if (
      data.sectorRelativeStrength === null ||
      data.sectorMomentum === null ||
      data.sectorBreadth === null ||
      data.stockRelativeStrength === null ||
      data.benchmarkRelativeStrength === null
    ) {
      warnings.push(
        "Sector rotation scores missing — will reject during detection if unresolved."
      );
    }

    const invalidBar = candles.find(
      (c) =>
        !Number.isFinite(c.open) ||
        !Number.isFinite(c.high) ||
        !Number.isFinite(c.low) ||
        !Number.isFinite(c.close)
    );
    if (invalidBar) {
      errors.push("Data quality failure — invalid OHLC values present.");
    }

    return {
      valid: errors.length === 0,
      errors,
      warnings,
    };
  }
}

export function createSectorRotationValidator(
  config?: Partial<SectorRotationConfig>
): SectorRotationValidator {
  return new SectorRotationValidator(
    config ?? DEFAULT_SECTOR_ROTATION_CONFIG
  );
}
