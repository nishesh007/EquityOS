...
/** Relaxed swing scoring when strict filters produce zero candidates */
export function scoreSwingRelaxed(metrics: LiveMetricsRecord): CategoryScanCandidate | null {
  if (!hasLiveQuote(metrics) || metrics.has_live_technicals !== 1) return null;
  ...
  if (trendScore === null) return null;
  if (trendScore < 45 && relativeStrength < 52 && fundamentalScore === null) return null;

  const side: "Long" | "Short" = trendScore >= 50 ? "Long" : "Short";
  const score =
    trendScore * 0.35 +
    (fundamentalScore ?? 48) * 0.25 +
...
export const SWING_PREFETCH_LIMIT = 80;

export function selectSwingPrefetchSymbols(
  metricsRows: LiveMetricsRecord[],
  limit = SWING_PREFETCH_LIMIT
): string[] {
  return metricsRows
    .filter((row) => hasLiveQuote(row))
    .map((row) => {
      const changePercent = Math.abs(num(row, "change_percent") ?? 0);
      const volume = typeof row.volume === "number" ? row.volume : 0;
      return {
        symbol: String(row.symbol ?? "").toUpperCase(),
        score: changePercent * 10 + Math.log10(volume + 1) * 5,
      };
    })
...
export function selectTopCandidatesWithFallback(
  category: OpportunityCategory,
  candidates: CategoryScanCandidate[],
  fallbackCandidates: CategoryScanCandidate[]
): CategoryScanCandidate[] {
  const top = selectTopCandidates(category, candidates);
  if (top.length > 0) return top;
  return selectTopCandidates(category, fallbackCandidates);
}
