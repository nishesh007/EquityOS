...
function buildCategoryCandidates(
  ...
  const top = selectTopCandidates(category, rescored);
  ...
  return top.map((candidate, index) => {
    ...
    return toOpportunityCandidate(candidate, index + 1, price, quote, atr);
  });
}
...
async function executeScan(force = false): Promise<ScanResult> {
  ...
  try {
    ...
    const shortlistSymbols = collectShortlistSymbols(categoryShortlists);

    const shortlistRows = quoteMetricsRows.filter((row) =>
      shortlistSymbols.includes(String(row.symbol ?? "").toUpperCase())
    );

    const needsFundamentals = shortlistRows.some((row) => {
      const symbol = String(row.symbol ?? "").toUpperCase();
      return categoryShortlists.swing.some((c) => c.symbol.toUpperCase() === symbol) ||
        categoryShortlists.ai_high_conviction.some((c) => c.symbol.toUpperCase() === symbol);
    });
